...
def executa_mdlo_chrg_icmo_prjt(params: list) -> None:
    """
    Função wrapper para executar a base tbciar_re_mdlo_chrg_icmo_prjt
    ...
    df = projeta_curva_incremental(du=du, data_max=data_max, suffixo="chrg")
    du.write_by_name(df, "CURVA_INC_CGB", "internal", partitions=["cd_dia_crte", "dh_crga"], mode="overwrite")
    df.unpersist()
...
if __name__ == "__main__":
    ...
    spark = (
        ...
        .appName("SPARK_prod_prazo_tbciar_re_mdlo_chrg_icmo_prjt")
        .enableHiveSupport()
        .getOrCreate()
    )
    ...
    executa_mdlo_chrg_icmo_prjt(sys.argv[1:])
