...
class CruzamentoChargeback:
    DATA_LIMITE: str = "20220101"
    DIAS_DE_CALCULO: int = 180
    ...
    def _busca_maior_data(
        ...
        """
        Busca o ultimo dado inserido na tabela de cruzamento de chargeback 
        ...
        if data_min is None:
            if self.du.is_table_available("CHARGEBACK", "internal"):
                data_min = max([v["cd_dh_incl_rgst"] for v in self.du.get_partition_values("CHARGEBACK", "internal")])
    ...
    def _salva_dados(self, df: DataFrame) -> None:
        """
        Dado um dataframe com dados de cruzamento de chargebacks salva na tabela.
    
        :param df (DataFrame): Dados novos que serao salvos na tabela
        """
        from utils.renames import renomeia_para_padrao
        self.du.write_by_name(renomeia_para_padrao(df), "CHARGEBACK", "internal", partitions="cd_dh_incl_rgst", mode="overwrite")

    def _busca_novos_chargebacks(self, data_min: str, data_max: str) -> DataFrame:   
        """
        Dados a entrada da uma particao inserida na tabela de cruzamento 
        busca por novos chargebacks na tabela de transacao, cruza dados 
        ...
        # lista os dados de chargeback
        cgb = (
            self.raw_trns.filter(
                (F.col("cd_dh_incl_rgst") >= int(data_min))
                & (F.col("cd_dh_incl_rgst") <= int(data_max))
                & (F.col("cd_tipo_lncm") == 8)
                & (F.col("dt_cptr_trns") >= min_trns)
            )
            .select(
                "nu_sqnl_agnd_trns",
                "cd_dh_incl_rgst",
                F.trim(F.col("nu_trns")).alias("nu_chrg"),
                F.trim(F.col("cd_idtf_trns_orig")).alias("nu_trns_orig"),
                F.to_date(F.col("dt_atzc_trns")).alias("dt_trns"),
                F.to_date(F.col("dt_cptr_trns")).alias("dt_chrg"),
                "cd_bndr",
                "dc_mtvo_ajst",
                "nu_ec",
                "nu_cnpj_cpf_recr",
                "cd_moda",
                F.col("vl_brto_trns_recb").alias("vl_brto_chrg"),
                F.col("vl_lqdo_trns").alias("vl_lqdo_chrg"),
                "nu_idtr_trns",
                "in_rbrp"
            )
        )

        # se existir uma base de chargeback, pega os últimos 540 dias antes da data minima
        # e remove os chargebacks já processados anteriorment
        if self.du.is_table_available("CHARGEBACK", "internal"):
            df_pre = (
                self.du.get_by_name("CHARGEBACK", "internal")
                .filter(
                ...
                .select("nu_chrg", F.lit(1).alias("in_pp"))
                .distinct()
                .checkpoint()
            )
            cgb = cgb.join(df_pre, on="nu_chrg", how="left").filter(F.col("in_pp").isNull()).drop("in_pp")

        # remove duplicatas da base restante
        windowSpec = Window.partitionBy("nu_chrg", "cd_dh_incl_rgst")
        df_chargebacks = (
            cgb.withColumn("nu_max", F.max("nu_sqnl_agnd_trns").over(windowSpec))
            ...
            .withColumn("min_partition", F.min("cd_dh_incl_rgst").over(Window.partitionBy("nu_chrg")))
        ...
        trns = (
            self.raw_trns.filter(
                ...
                & (F.col("cd_tipo_lncm").isin([1, 2, 3, 6, 7]))
            )
            .select(
                "cd_tipo_lncm",
            ...
            .join(
                df_chargebacks.select("nu_trns_orig").distinct(),
                on=["nu_trns_orig"],
                how="inner",
            )
            .persist()
        ...
        df_chargebacks = (
            ...
            .select("*", F.datediff(F.col("dt_chrg"), F.col("dt_trns")).alias("nu_dia_vnda"))
        )

        # calcula o valor a liquidar para cada chargeback
        df_liquidar = (
            df_chargebacks.select("nu_chrg", "dt_chrg", "nu_trns_orig").alias("a")
            .join(
                df_valor.alias("b"),
                on=(
                    (F.col("a.nu_trns_orig") == F.col("b.nu_trns_orig"))
                    & (F.col("a.dt_chrg") <= F.col("b.dt_vncm_unde_rcbv"))
                ),
                how="left"
            )
            .groupBy("nu_chrg")
        ...
        df_chargebacks = (
            df_chargebacks.join(df_liquidar, on="nu_chrg", how="left")
            .select(
                list(df_chargebacks.columns)
                + [
        ...
        df_chargebacks = df_chargebacks.join(self.bndr, on="cd_bndr", how="left").drop("cd_bndr")
        df_chargebacks = df_chargebacks.join(self.ec, on="nu_ec", how="left")

        return df_chargebacks
    ...
    def executar(
        ...
        # transforma os dados
        df = self._busca_novos_chargebacks(data_min, data_max)

        # salva os resultados
        self._salva_dados(df)


def executa_chrg(params: list) -> None:
    """
    Função wrapper para executar a base de Chargeback
    ...
    dados_chargeback = CruzamentoChargeback(du)
    dados_chargeback.executar(data_min, data_max)
...
if __name__ == "__main__":
    ...
    spark = (
        ...
        .appName("SPARK_prod_prazo_tbciar_re_chrg")
